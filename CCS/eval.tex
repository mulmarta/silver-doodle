...
We remark that \cite{hashimoto2021cmpke} uses a more coarse-grained metric, namely, the total sender and receiver bandwidth for all receivers (which corresponds to the server load). Their metric can clearly be recovered from ours.
...
Therefore, we compare the minimum and maximum bandwidth over all possible execution histories.
...
In summary, the results show that \saik achieves the lowest bandwidth required from a single client, while \protCMPKE has the lowest total server bandwidth. (Hence, both protocols meet their design goals.) For instance, for 10K parties, \protCMPKE requires a client to upload $783$KB of data, while in scenarios close to the best-case (which we believe to occur most of the time), the sender or receiver bandwidth of \saik is roughly $5$KB. On the other hand, the server bandwidth is roughly $25$MB for \protCMPKE and $30$MB for \saik.
