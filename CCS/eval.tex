...
We break down the communication complexity into the \emph{sender bandwidth}, i.e., the size of a packet uploaded to the
server, and the (average) \emph{receiver bandwidth}, i.e., the size of a (personalized) packet downloaded by a single
receiver. 
...
load). Their metric can clearly be recovered from ours. We choose to split the bandwidth because it allows us to compare
not only the total network bandwidth but also the maximum bandwidth required by a single client. This metric is of
interest as a protocol with smaller maximum client bandwidth allows for weaker clients overall.

There are some caveats when calculating the bandwidth requirements for \saik (and \protITK) due to the underlying tree
structure:
...
protocols we compare the average receiver bandwidth over all members of the group. (For \protITK and \protCMPKE it makes
no difference.) For \saik, this is equivalent to considering the worst-case for receivers, as approximately half of the
receivers always need to download all public keys as they are in the opposite subtree of the sender relative to the root
of the tree.
  ...
  \begin{minipage}[t]{.48\textwidth}
    ...
    \begin{minipage}[t]{\linewidth}
    	\begin{tabulary}{\linewidth}{|l|l|l|l|l|}
    		...
    		\multirow{2}{*}{\parbox{1.5cm}{(Average)\\receiver}} 
...
For receivers, \protITK is much worse than \saik and \protCMPKE. For
