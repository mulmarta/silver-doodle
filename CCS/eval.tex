...
leaves; see \cref{fig:tree-full}. However, in the worst case there can be $N$ recipients. This happens e.g. when all
...
\end{table*}
\paragraph{Results.}
We computed the bandwidth for all protocols using the formulas in \cref{tab:bandwidth1,tab:bandwidth2}. Further, this is
...
We highlight some interesting features of the plots. For concreteness, we fix $N = 10K$ users.
For senders, \saik is always at least as good as the other protocols. It requires between $83\%$ and only $55\%$ of the bandwidth of \protITK (due to the use of mmPKE). Compared to \protCMPKE, the sender bandwidth of \saik is the same for the worst-case execution history and for the best-case execution, \saik needs as little as $0.45\%$ of the bandwidth, e.g. in a
