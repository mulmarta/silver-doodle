...
server, and the (maximum) \emph{receiver bandwidth}, i.e., the size of a (personalized) packet downloaded by a single
receiver. 
...
load). An upper bound on their metric can easily be recovered from ours. We choose to split the bandwidth because it
...
execution) of \protITK's. On the other hand, \protCMPKE is (unsurprisingly) the best for receivers.  \saik requires up
